...
public class RabbitMQConsumer extends DefaultConsumer {
    ...
    protected void doStart() throws Exception {
        ...
        log.debug("Queue name {}", endpoint.getQueue());
    ...
    protected void doStop() throws Exception {
        ...
        try {
        	channel.queueDelete(endpoint.getQueue());
        } catch (IOException e) {
    		log.error("Problem deleting queue {}", endpoint.getQueue());
    	}
        log.info("Deleted queue {}", endpoint.getQueue());
        channel.close();
    ...
    private void createExchange() throws IOException {
        final Settings settings = Bootstrap.getInstance().getSettings();
    	final boolean createExchange = (settings.getBoolean("amqp.createExchange") == null)?false:settings.getBoolean("amqp.createExchange");
    	if (createExchange){
			// FIXME https://issues.apache.org/jira/browse/CAMEL-6909
			channel.exchangeDeclare(endpoint.getExchangeName(),
					// TODO report and parametrize
					"topic", endpoint.isDurable(),
					endpoint.isAutoDelete(),
					new HashMap<String, Object>());
    	} else {
    		try {
    			channel.exchangeDeclarePassive(endpoint.getExchangeName());
    		}
    		catch (IOException e){
    			log.error("\n\nExchange: " + endpoint.getExchangeName() + " does not exist!\n" + 
    					"Please check that the correct \"exchangeName\" is specified in the settings.\n" +
    					"If you wish to create a new exchange, you must do that manually or by setting \"createExchange\" to \"true\".\n");
    		}
    	}
    }
    ...
    class RabbitConsumer extends com.rabbitmq.client.DefaultConsumer {
        ...
        public void handleDelivery(String consumerTag,
            ...
            try {
                ...
                // FIXME https://issues.apache.org/jira/browse/CAMEL-6767
